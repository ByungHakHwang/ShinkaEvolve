...
Version 3: Completely naive baseline with no bias toward any specific rule.

This allows ShinkaEvolve to discover the true general pattern without
being constrained by known special cases like Pieri.
"""
...
# ============================================
# HELPER FUNCTIONS
# Available to LLM but not mandatory to use
# ============================================
...
def hook_length(partition: Partition, cell: Cell) -> int:
    """
    Hook length: h(r,c) = a(r,c) + l(r,c) + 1
    """
    arm, leg = arm_leg_length(partition, cell)
    return arm + leg + 1
...
def compute_ABC_sets(
    ...
    DESIGN PHILOSOPHY FOR THIS INITIAL VERSION:
    - Start with the most naive possible baseline
    - No assumptions about special cases
    - No bias toward any particular pattern
    - Let the data guide the discovery completely
    
    This initial implementation is intentionally simplistic and likely
    incorrect for most cases. ShinkaEvolve will discover the true pattern
    from the ground truth data without preconceptions.
    ...
    # NOTE FOR EVOLUTION:
    # The above rules are extremely naive and almost certainly wrong.
    # The correct rules likely involve:
    # - Arm and leg lengths
    # - Relationships between the three partitions
    # - Geometric properties of skew shapes
    # - Row and column indices of skew partitions
    # - Possibly corner cells, boundary cells, etc.
    #
    # Do NOT forget that A, B, C is sets of cells in nu, lambda, mu, respectively.
    # DO NOT assume any special cases (like single-row μ).
    # Discover the general pattern that works for ALL cases.
...
def run_experiment(**kwargs):
    ...
    # Compute using the general function
